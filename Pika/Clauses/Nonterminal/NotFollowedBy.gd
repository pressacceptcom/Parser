tool
class_name PressAccept_Parser_Pika_NotFollowedBy
...
static func notfollowedby_instantiate(
		init_subclause: Clause) -> PressAccept_Parser_Pika_NotFollowedBy:
...
func __output(
	...
	if _being_output:
		return "\n" + prefix + 'NotFollowedBy:' + str(get_instance_id())
...
func __get_script():
	...
	return 'res://addons/PressAccept/Parser/Pika/Clauses/Nonterminal/NotFollowedBy.gd'
		...
		self[THROWABLE].throw(
			{
				...
				Exception.STR_EXCEPTION_MESSAGE : \
					'Subclause always matches zero characters, so this ' \
						+ 'clause will never match anything: ' + str(self)
			}
		)
