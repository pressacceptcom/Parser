tool
class_name PressAccept_Parser_Pika_First
...
func __init(
	...
	if STR_SUBCLAUSES in init_args and init_args[STR_SUBCLAUSES].size() < 2:
...
func __output(
	...
	if _being_output:
		return "\n" + prefix + 'First:' + str(get_instance_id())
...
func __get_script():
	...
	return 'res://addons/PressAccept/Parser/Pika/Clauses/Nonterminal/First.gd'
...
func determine_whether_can_match_zero_chars(): # -> void
	...
	for index in range(labeled_subclauses.size()):
		# Up to one subclause of a First clause can match zero characters, and
		# if present, the subclause that can match zero characters must be the
		# last subclause
		if labeled_subclauses[index].clause.can_match_zero_chars:
			can_match_zero_chars = true
			if index < labeled_subclauses.size() - 1:
				return self[THROWABLE].throw(
					{
						Exception.STR_EXCEPTION_CODE    : 'Illegal Argument',
						Exception.STR_EXCEPTION_MESSAGE : 'Subclause ' \
							+ str(index) + ' of First can match zero ' \
							+ 'characters, which means subsequent subclauses ' \
							+ 'will never be matched: ' + str(self)
					}
				)
			break
...
func match(
	...
	for subclause_index in range(labeled_subclauses.size()):
		var subclause          : Clause    = \
			labeled_subclauses[subclause_index].clause
		var subclause_memo_key : Array   = \
			[ subclause, memo_key[MemoTable.INT_START_POS_INDEX] ] # : MemoKey
